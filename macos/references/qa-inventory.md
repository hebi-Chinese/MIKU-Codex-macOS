...
- Live verification after explicit diagnostic reload returns version `1.3.8` and `pass: true`; normal watcher/application paths do not reload the page.
- The MIKU preset additionally requires `mikuContractPass: true`, adapter contract `miku-native-v2-2026-07-20.8`, renderer reconciliation contract `stream-safe-v2`, a fixed persistent art layer, 15 support phrases, 4 permission presentations, at least 56 SVG symbols, native `HanziPen SC` / `Hannotate SC` preview typography, `previewArtTypographyPass: true`, `permissionArtTypographyPass: true` for an open permission menu, loaded bundled WOFF2 fallback, loaded side-chat art, and complete coverage of every open side-chat panel. A wallpaper-only, wrong-font, white-side-chat, or GPT-streaming flicker state must fail.
