...
- Live verification after `Page.reload` returns version `1.3.3` and `pass: true`.
- The MIKU preset additionally requires `mikuContractPass: true`, adapter contract `miku-native-v2-2026-07-20.3`, renderer reconciliation contract `stream-safe-v1`, 15 support phrases, 4 permission presentations, at least 56 SVG symbols, loaded bundled art typography, loaded side-chat art, and complete coverage of every open side-chat panel. A wallpaper-only, ordinary-font, white-side-chat, or GPT-streaming flicker state must fail.
