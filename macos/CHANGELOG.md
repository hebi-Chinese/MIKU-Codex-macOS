...
- 按用户最终确认，恢复主题最初已通过视觉验收的 `LXGW WenKai GB` 艺术字体资产与权限标题 `600`、说明 `500` 的原始视觉字重；不再使用 1.3.6 的 Ma Shan Zheng 或 1.3.5 的 ZCOOL KuaiLe。
- 恢复的是已发布历史中的精确 WOFF2（SHA-256 `1a8a9eb796621aac0a3ce8c2e4a74384bc8da05e0de24887bf29d9a6ab9bc734`），并新增回归断言，避免未来再次把“最初艺术体”误替换为其他手写字体。
- 保留固定 `codex-dream-skin-art-layer`、语义化 DOM 对账和 `stream-safe-v2`；这次字体回切不会回退流式回答防闪屏修复。
- 版本提升为 `1.3.7`，MIKU 安装契约提升为 `miku-native-v2-2026-07-20.7`。

### 说明

- 该字体继续只服务应援语、权限视觉文案与灵感标题；正文、任务、代码和原生控件仍使用稳定 UI 字体。
