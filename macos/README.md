...
| Persistent themed launch entries | `~/Desktop/MIKU Codex.app`, `~/Applications/MIKU Codex.app` |

Both copies of `MIKU Codex.app` are the same small user-local native applet launcher. Their executable
is a LaunchServices-compatible universal Mach-O generated by macOS `osacompile`; it invokes the
stable, auditable shell engine rather than exposing a text script as the app executable. They start the signed official
Codex executable through the verified loopback-only injector path and repair
an ordinary launch that lacks CDP. It does not copy, patch, re-sign, or replace
the official app, and it does not keep Codex alive after you quit. Launcher failures are written to
the stable Application Support logs and surfaced with an explicit macOS alert. Both launchers
use the same bright, project-owned `MIKUCodex.icns`, generated from the auditable
`assets/miku-codex-app-icon.svg` source instead of a downloaded character image. Pin this
entry if you want every future launch to restore the theme automatically; the
original official Dock entry cannot receive Chromium flags without crossing
the app-bundle boundary.
...
For the private MIKU build, package the currently selected MIKU theme together with the
engine and three Chinese double-click entries:

```bash
./scripts/build-miku-client-release.sh \
  "$HOME/Downloads/MIKU Codex 个人版-$(date +%Y-%m-%d).zip" \
  --theme-dir "$HOME/Library/Application Support/CodexDreamSkinStudio/theme"
```

The MIKU archive does not contain a prebuilt `MIKU Codex.app`: the installer regenerates the
native applet on the destination Mac so no source-machine home path is embedded. It also writes
an adjacent `.sha256` file. This is a private, non-notarized package; public distribution still
requires asset-rights review plus Developer ID signing and Apple notarization.
...
ZCOOL KuaiLe in the official Google Fonts repository, and embeds it into the renderer payload. Its OFL license,
