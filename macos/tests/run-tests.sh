...
if ! /usr/bin/grep -F -q 'KeepAlive -bool false' "$ROOT/scripts/common-macos.sh"; then
  printf 'Codex CDP launch must use a non-keepalive LaunchAgent.\n' >&2
  exit 1
fi
if ! /usr/bin/grep -F -q 'launchctl bootstrap "gui/$uid" "$codex_job_plist"' \
  "$ROOT/scripts/common-macos.sh"; then
  printf 'Codex CDP launch must bootstrap the explicit one-shot LaunchAgent.\n' >&2
  exit 1
fi
if /usr/bin/sed -n '/^launch_codex_with_cdp()/,/^}/p' "$ROOT/scripts/common-macos.sh" \
  | /usr/bin/grep -F -q '/usr/bin/open -na'; then
  printf 'Codex CDP launch still relies on LaunchServices, which may discard Chromium flags.\n' >&2
  exit 1
fi
if /usr/bin/grep -F -q '/usr/bin/open -na "$CODEX_BUNDLE" --args --remote-debugging' \
  "$ROOT/scripts/start-dream-skin-macos.sh"; then
  printf 'start script still creates an unverified second Codex instance through LaunchServices.\n' >&2
  exit 1
fi
...
/usr/bin/env -u HOME /bin/bash -c '. "$1/scripts/common-macos.sh"; [ -n "$HOME" ] && [ "$SKIN_VERSION" = "1.3.6" ]' _ "$ROOT"
