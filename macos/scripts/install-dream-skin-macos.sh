...
if [ "$CREATE_LAUNCHERS" = "true" ]; then
  ...
  "$SCRIPT_DIR/install-miku-launcher-macos.sh" --port "$PORT" --target "$HOME/Applications/MIKU Codex.app"
  "$SCRIPT_DIR/install-miku-launcher-macos.sh" --port "$PORT" --target "$HOME/Desktop/MIKU Codex.app"
fi
...
printf 'For future launches, open MIKU Codex.app from your Desktop or user Applications folder.\n'
