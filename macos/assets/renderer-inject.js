((cssText, artDataUrl, sideChatArtDataUrl, themeConfig) => {
  ...
  const sideChatArtUrl = createArtUrl(sideChatArtDataUrl);
  ...
  previous?.mikuA4Adapter?.cleanup?.();
  ...
  const syncMikuA4 = ({ home = null, shellMain = null } = {}) => {
    ...
    if (!mikuA4Adapter) {
      const factory = window[MIKU_A4_FACTORY_KEY];
      if (typeof factory !== "function") return null;
      mikuA4Adapter = factory({ document, window, theme: THEME });
      if (window[STATE_KEY]) window[STATE_KEY].mikuA4Adapter = mikuA4Adapter;
    }
    return mikuA4Adapter.sync({ home, shellMain });
  };
  ...
  const syncMikuProjectMark = () => {
    ...
    const liveNativeIcons = new Set();

    for (const row of rows) {
      const slot = row.querySelector('[data-sidebar-project-drop-zone="project-icon"]');
      if (!slot) continue;
      const projectKey = row.getAttribute("data-app-action-sidebar-project-label")
        || row.getAttribute("data-app-action-sidebar-project-id")
        || row.textContent?.replace(/\s+/g, " ").trim();
      const recipe = mikuA4Adapter?.projectRecipeFor?.(projectKey) || "neutral-folder";
      const nativeIcon = [...slot.children].find((node) =>
        node.matches?.("svg") && !node.closest?.(`.${markClass}`),
      );
      nativeIcon?.classList.add(nativeClass);
      if (nativeIcon) liveNativeIcons.add(nativeIcon);

      let mark = [...slot.children].find((node) => node.classList?.contains(markClass));
      if (!mark) {
      ...
      if (mark.dataset.iconRecipe !== recipe) {
        mark.dataset.iconRecipe = recipe;
        mark.innerHTML = `<svg viewBox="0 0 24 24" focusable="false">
          <use class="dream-miku-mark-orbit" href="#dream-icon-signal-orbit"></use>
          <g class="dream-miku-mark-core"><use href="#dream-icon-project-${recipe}"></use></g>
          <circle class="dream-miku-mark-pulse" cx="12" cy="12" r="9.2"></circle>
        </svg>`;
      }
      liveMarks.add(mark);
    }

    for (const mark of document.querySelectorAll(`.${markClass}`)) {
      if (!liveMarks.has(mark)) mark.remove();
    }
    for (const icon of document.querySelectorAll(`.${nativeClass}`)) {
      if (!liveNativeIcons.has(icon)) icon.classList.remove(nativeClass);
    }
  };
  ...
  const applyRootState = (root) => {
    ...
    if (sideChatArtUrl) {
      ...
      root.style.removeProperty("--dream-miku-side-chat-art");
    }
  ...
  const syncRouteState = (shell, { layout = false } = {}) => {
    ...
    if (observedShellMain !== shellMain) {
      observedShellMain?.classList?.remove("dream-skin-main-surface");
      resizeObserver?.disconnect();
      resizeObserver?.observe(shellMain);
      observedShellMain = shellMain;
      layout = true;
    }
    shellMain.classList.add("dream-skin-main-surface");
    ...
    if (created || chrome.dataset.dreamIconRevision !== ICON_REVISION) {
      chrome.innerHTML = `${ICON_SPRITE}
        <div class="dream-skin-brand">
          <svg class="dream-skin-portal-mark" viewBox="0 0 24 24" aria-hidden="true"><use href="#dream-icon-wave-heart"></use></svg>
          <span><b></b><small></small></span>
        </div>
        <div class="dream-skin-status"><svg viewBox="0 0 24 24" aria-hidden="true"><use href="#dream-icon-heart"></use></svg><span></span></div>
        <div class="dream-skin-quote"></div>
        <div class="dream-skin-particles">
          <svg viewBox="0 0 24 24"><use href="#dream-icon-heart"></use></svg>
          <svg viewBox="0 0 24 24"><use href="#dream-icon-heart"></use></svg>
          <svg viewBox="0 0 24 24"><use href="#dream-icon-heart"></use></svg>
          <svg viewBox="0 0 24 24"><use href="#dream-icon-heart"></use></svg>
        </div>
        <div class="dream-miku-motifs">
          <svg class="dream-miku-motif dream-miku-motif--voice" viewBox="0 0 24 24"><use href="#dream-icon-wave-heart"></use></svg>
          <svg class="dream-miku-motif dream-miku-motif--notes" viewBox="0 0 24 24"><use href="#dream-icon-twin-note"></use></svg>
          <svg class="dream-miku-motif dream-miku-motif--words" viewBox="0 0 24 24"><use href="#dream-icon-word-bloom"></use></svg>
        </div>
        <div class="dream-skin-orbit"></div>`;
      chrome.dataset.dreamIconRevision = ICON_REVISION;
      chromeParts = null;
    }
  ...
  const cleanup = () => {
    ...
    document.querySelectorAll(".dream-miku-project-mark").forEach((node) => node.remove());
    document.querySelectorAll(".dream-miku-native-icon").forEach((node) => node.classList.remove("dream-miku-native-icon"));
    mikuA4Adapter?.cleanup?.();
    mikuA4Adapter = null;
    ...
    if (state?.sideChatArtUrl) URL.revokeObjectURL(state.sideChatArtUrl);
    delete window[MIKU_A4_FACTORY_KEY];
  ...
  if (previous?.sideChatArtUrl && previous.sideChatArtUrl !== sideChatArtUrl) {
    URL.revokeObjectURL(previous.sideChatArtUrl);
  }
  ...
  __DREAM_SKIN_SIDE_CHAT_ART_JSON__,
