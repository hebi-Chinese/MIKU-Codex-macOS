...
- Live verification after explicit diagnostic reload returns version `1.3.7` and `pass: true`; normal watcher/application paths do not reload the page.
- The MIKU preset additionally requires `mikuContractPass: true`, adapter contract `miku-native-v2-2026-07-20.7`, renderer reconciliation contract `stream-safe-v2`, a fixed persistent art layer, 15 support phrases, 4 permission presentations, at least 56 SVG symbols, loaded bundled original LXGW WenKai GB display typography, `permissionArtTypographyPass: true` for an open permission menu, loaded side-chat art, and complete coverage of every open side-chat panel. A wallpaper-only, ordinary-font, white-side-chat, or GPT-streaming flicker state must fail.
