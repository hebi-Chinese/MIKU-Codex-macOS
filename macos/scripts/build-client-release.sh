...
/usr/bin/printf '%s\n' \
  ...
  'set -euo pipefail' \
  'ROOT="$(cd "$(dirname "$0")" && pwd -P)"' \
  'ENGINE="$ROOT/.codex-dream-skin-studio"' \
  ...
  '/usr/bin/printf "%s\\n" "MIKU Codex 已安装。请从 MIKU Codex.app 启动并按部署提示词验收。"' \
  > "$CLIENT_ROOT/安装 Codex 主题编辑器.command"

/usr/bin/printf '%s\n' \
  'MIKU Codex for macOS 1.3.8' \
  '' \
  '推荐方式：把这个完整 ZIP、你喜欢的图片和“给 Codex 的部署提示词.md”一起发给自己的 Codex。' \
  '' \
  '手动方式：双击“安装 Codex 主题编辑器.command”，选择 preset-miku-love-words，并从 MIKU Codex.app 启动。' \
  '' \
