...
CODEX_APP_PLIST="$STATE_ROOT/codex-app.plist"
...
launch_injector_daemon() {
  ...
  : > "$INJECTOR_ERROR_LOG"
  /bin/launchctl remove "$INJECTOR_JOB_LABEL" >/dev/null 2>&1 || true
  # `launchctl remove` is asynchronous. Re-submitting the same label before
  # the old job disappears can return its dying PID and leave stale state.
  local remove_deadline=$((SECONDS + 3))
  while /bin/launchctl print "gui/$(/usr/bin/id -u)/$INJECTOR_JOB_LABEL" >/dev/null 2>&1 \
    && [ "$SECONDS" -lt "$remove_deadline" ]; do
    /bin/sleep 0.05
  done

  # The injector must outlive the terminal or menu-bar process that started it.
  # A submitted job is scoped to the injector only: it never launches Codex,
  # while launchd gives us a stable lifecycle and a removable process boundary.
  ...
  # Some managed macOS environments reject submitted jobs. Fall back to a
  # detached process with every inherited terminal stream disconnected.
  /bin/launchctl remove "$INJECTOR_JOB_LABEL" >/dev/null 2>&1 || true
  /usr/bin/nohup "$NODE" "$INJECTOR" --watch --port "$port" --theme-dir "$THEME_DIR" \
    </dev/null >>"$INJECTOR_LOG" 2>>"$INJECTOR_ERROR_LOG" &
  pid="$!"
  /bin/sleep 0.08
  if [ -n "$pid" ] && /bin/kill -0 "$pid" 2>/dev/null; then
    printf '%s\n' "$pid"
...
release_codex_launchd_job() {
  local uid
  uid="$(/usr/bin/id -u)"
  /bin/launchctl bootout "gui/$uid/$CODEX_APP_JOB_LABEL" >/dev/null 2>&1 || true
...
launch_codex_with_cdp() {
  local port="$1"
  local uid
  local codex_job_plist="$CODEX_APP_PLIST"
  : > "$APP_LOG"
  : > "$APP_ERROR_LOG"
  ensure_state_root
  release_codex_launchd_job
  ...
  # LaunchServices may silently discard Chromium flags.  An explicit
  # non-keepalive LaunchAgent starts the verified executable directly, keeps
  # the arguments intact, and still lets a normal user quit stay quit.
  /usr/bin/plutil -create xml1 "$codex_job_plist"
  /usr/bin/plutil -insert Label -string "$CODEX_APP_JOB_LABEL" "$codex_job_plist"
  /usr/bin/plutil -insert ProgramArguments -array "$codex_job_plist"
  /usr/bin/plutil -insert ProgramArguments.0 -string "$CODEX_EXE" "$codex_job_plist"
  /usr/bin/plutil -insert ProgramArguments.1 -string '--remote-debugging-address=127.0.0.1' "$codex_job_plist"
  /usr/bin/plutil -insert ProgramArguments.2 -string "--remote-debugging-port=$port" "$codex_job_plist"
  /usr/bin/plutil -insert EnvironmentVariables -dictionary "$codex_job_plist"
  /usr/bin/plutil -insert EnvironmentVariables.HOME -string "$HOME" "$codex_job_plist"
  /usr/bin/plutil -insert RunAtLoad -bool true "$codex_job_plist"
  /usr/bin/plutil -insert KeepAlive -bool false "$codex_job_plist"
  /usr/bin/plutil -insert ProcessType -string Interactive "$codex_job_plist"
  /usr/bin/plutil -insert StandardOutPath -string "$APP_LOG" "$codex_job_plist"
  /usr/bin/plutil -insert StandardErrorPath -string "$APP_ERROR_LOG" "$codex_job_plist"
  /bin/chmod 600 "$codex_job_plist"
  /bin/launchctl bootstrap "gui/$uid" "$codex_job_plist"
}
